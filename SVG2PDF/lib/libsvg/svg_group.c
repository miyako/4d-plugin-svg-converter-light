				 ...
				 const char	**attributes)
{
    ...
    if (view_box_str)
    {
	status = (svgint_status_t)_svg_element_parse_view_box (view_box_str,
	...
	if (aspect_ratio_str)
	    status = (svgint_status_t)_svg_element_parse_aspect_ratio (aspect_ratio_str, &group->view_box);
    }
...
_svg_group_grow_element_by (svg_group_t *group, int additional)
{
    ...
    new_element = (svg_element_t **)realloc (group->element,
