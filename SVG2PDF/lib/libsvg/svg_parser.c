			       ...
			       const xmlChar	**attributes_unsigned)
{
    ...
    if (parser->status) {
	if (parser->status == (svgint_status)SVGINT_STATUS_UNKNOWN_ELEMENT)
			...
			const svg_parser_cb_t	*cb)
{
    ...
    state = (svg_parser_state_t *)malloc (sizeof (svg_parser_state_t));
    if (state == NULL)
...
_svg_parser_new_svg_group_element (svg_parser_t *parser, svg_element_t **group_element)
{
    ...
    status = (svg_status_t)_svg_element_create (group_element,
			      ...
			      svg_element_type_t type)
{
    ...
    status = (svg_status_t)_svg_element_create (child_element, type,
