...
{
    *path = (svg_path_t *)malloc (sizeof (svg_path_t));
    if (*path == NULL)
...
_svg_path_add_from_str (svg_path_t *path, const char *path_str)
{
	...
	while (1) {
	    status = (svg_status_t)_svg_str_parse_csv_doubles (s, arg, cmd_info->num_args, &end);
	    s = end;
...
_svg_path_op_buf_create (void)
{
    ...
    op = (svg_path_op_buf_t *)malloc( sizeof (svg_path_op_buf_t));
...
_svg_path_arg_buf_create (void)
{
    ...
    arg_buf = (svg_path_arg_buf_t *)malloc (sizeof (svg_path_arg_buf_t));
